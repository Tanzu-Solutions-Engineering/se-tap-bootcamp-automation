...
resource "azurerm_kubernetes_cluster" "aks_win" {
  ...
  windows_profile {
    ...
    admin_password = random_string.windows_admin_password.result
  }
...
resource "local_file" "kubeconfig" {
  ...
  filename = pathexpand("~/.kube/${azurerm_kubernetes_cluster.aks_win.name}-config")
}

data "azurerm_public_ip" "aks" {
  name                = reverse(split("/", tolist(azurerm_kubernetes_cluster.aks_win.network_profile.0.load_balancer_profile.0.effective_outbound_ips)[0]))[0]
